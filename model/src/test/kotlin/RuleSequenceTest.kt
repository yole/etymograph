...
class RuleSequenceTest : QBaseTest() {
    ...
    @Test
    fun simpleSequence() {
        val qAiE = q.rule("* a > e / _i", name = "q-ai-e")
        val qSfF = q.rule("* s > 0 / _f") // inapplicable for this test
        val qWV = q.rule("* w > v / #_", name = "q-w-v")
        val seq = graph.addRuleSequence("ce-q", ce, q, listOf(qAiE.step(), qSfF.step(), qWV.step()))
        val ceWord = ce.word("waiwai", "waiwai")
        val qWord = q.word("vaiwe", "vaiwe")
    ...
    fun simpleSequencePhonemic() {
        val vowelLengthening = q.rule("* a > ā / _[+voice]",
            name = "q-vowel-lengthening")
        val rule1 = q.rule("* a > e / _i", name = "q-ai-e")
        val rule2 = q.rule("* z -> r")
        val rule3 = q.rule("- apply sound rule 'q-vowel-lengthening' to first vowel", name = "q-w-v")
        val seq = graph.addRuleSequence("ce-q", ce, q, listOf(rule1.step(), rule2.step(), rule3.step()))
        val ceWord = ce.word("suzja", "suzja")
        val qWord = q.word("surya", "surya")
        val link = graph.addLink(qWord, ceWord, Link.Origin)
        graph.applyRuleSequence(link, seq)
    ...
    fun optionalSteps() {
        ...
        val ceWord = ce.word("waiwai", "waiwai")
        val qWord = q.word("weiwei", "weiwei")
        val link = graph.addLink(qWord, ceWord, Link.Origin)
        graph.applyRuleSequence(link, seq)
        assertEquals(1, link.rules.size)
    }
    ...
    fun alternativeRules() {
        val qAO = q.rule("* a > o", name = "q-a-o")
        val qAI = q.rule("* a > i", name = "q-a-i")
        val seq = graph.addRuleSequence("ce-q", ce, q, listOf(qAO.step(alternative = qAI)))
        val ceWordA = ce.word("wawa", "wawa")
        val qWordO = q.word("wowo", "wowo")
        val link = graph.addLink(qWordO, ceWordA, Link.Origin)
        graph.applyRuleSequence(link, seq)
        assertEquals("q-a-o", link.rules[0].name)

        val ceWordI = ce.word("lala", "lala")
        val qWordI = q.word("lili", "lili")
    ...
    fun normalize() {
        ...
        val qVoiceless = q.rule("* [-sonorant,-continuant] > [+voice]", name = "q-voiceless")
        val seq = graph.addRuleSequence("ce-q", ce, q, listOf(qVoiceless.step()))
        val ceWord = ce.word("aklar", "aklar")
        val qWord = q.word("aglar", "aglar")
        val link = graph.addLink(qWord, ceWord, Link.Origin)
        graph.applyRuleSequence(link, seq)
        assertEquals(1, link.rules.size)
    }
