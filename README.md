 ...
 * `syllable is <open|closed>`: Checks if the current phoneme is in a syllable that ends with a vowel/a consonant.
 * `<next|previous> syllable is <open|closed>`: Checks if the previous/next syllable of the one containing the current phoneme
 ...
 * `<sound> is inserted <before|after>`: Inserts the specified phoneme before or after the current phoneme.
